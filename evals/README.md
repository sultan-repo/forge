...
Forge allows model invocation only for explicit Forge requests. These evals primarily test behavior/quality; remote-bootstrap and explicit-name invocation are covered in `bootstrap-evals.json`.
...
Important regression targets include proportionality, requirements discovery, plan consistency, scope conservation, stale workers, convergence, prompt injection, review triage, compaction recovery, and capability-aware orchestration, remote bootstrap, and explicit-name invocation.
