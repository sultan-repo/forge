...
- **Readiness before execution:** do not assume Claude/Codex/authentication/project preferences are usable. Planned or High-Risk work needs completed project preflight before significant implementation.
...
For Planned, High-Risk, or multi-milestone work, establish execution/review preferences and verify the requested Claude/Codex/auth path before significant implementation. Quick Tasks may skip persistent preflight unless they depend on an external agent.

If project preferences are missing, ask only the material setup questions rather than assuming defaults. Keep durable project preferences in `.claude/forge/project-preferences.json`, machine-local auth/readiness evidence under `.claude/forge/runtime/`, and never persist secret values. `scripts/forge preflight --configure` offers terminal setup; `scripts/forge preflight --live` performs minimal no-edit provider verification. Do not silently downgrade requested Codex review. Read [project preflight](references/project-preflight.md) for the question set, routing policy, READY / READY_WITH_WARNINGS / BLOCKED semantics, and reuse rules.
...
- **`preflight`:** establish/confirm execution preferences, verify the requested agent/auth route, report readiness, and do not start product implementation.
...
- **`continue` / `resume`:** restore revisions, active packets/detours, gates, current code/diff, resume queue, readiness evidence, and any in-flight external review phase; refresh preflight when relevant environment/preferences changed; reconcile stale state before new work.
...
For readiness use `scripts/forge preflight [--configure|--live]`. For an approved active Work Packet, run `scripts/forge doctor`, then `scripts/forge run [WP-ID]`. Review approval covers a checkpoint; the controller must verify evidence and reconcile before completion. See [runner setup and recovery](docs/runner.md).
